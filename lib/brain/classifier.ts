...
export interface CandidateFindingForClassification {
  ...
  // Thin MVP inputs coming from reasoning / verification-lite
  ...
  // Helpful operational fields for MVP
...
function deriveDisputeTarget(ruleFamily: string, provided?: DisputeTargetFinal | null): DisputeTargetFinal {
  if (provided) return provided

  const rf = ruleFamily.toLowerCase()

  if (rf.includes("network") || rf.includes("authorization") || rf.includes("adjudication")) {
    return "payer"
  }

  if (rf.includes("duplicate") || rf.includes("pricing") || rf.includes("coding") || rf.includes("upcoding")) {
    return "provider"
  }
...
function classifyFinding(input: CandidateFindingForClassification): ClassifiedFinding {
  ...
  const duplicateDisputeInProgress = Boolean(input.duplicate_dispute_in_progress)
  ...
  if (shadowCandidate || criticalFieldsMissing) {
    ...
    classificationReason = "Critical evidence missing or finding marked as shadow candidate."
  } else if (agreementStatus === "rejected" || agreementStatus === "insufficient_evidence") {
...
const mediumThreshold = dynamicThreshold - 0.20
  ...
  (severity === "high" || severity === "critical")
      ...
      truthLayerStatus = "manual_review"
      classificationReason = "Medium-confidence finding requires review before action."
    } else {
      finalClassification = "green"
      disputeStatus = "not_needed"
      truthLayerStatus = "not_required"
      classificationReason = "Confidence too low for action; finding cleared for MVP."
    }
  }
  ...
  const disputeTarget = finalClassification === "green" || finalClassification === "shadow"
    ? "none"
  ...
  const participationRewardEligible = false // deferred until savings event confirmation in MVP

  const plainLanguageExplanationFinal =
    input.plain_language_explanation?.trim() ||
    buildPlainLanguageExplanation(finalClassification, input.rule_family, estimatedSavings)
...
function buildPlainLanguageExplanation(
  ...
  if (finalClassification === "red") {
    return `We found a likely ${issue} issue on this bill. Do not pay yet while we prepare a correction request. Estimated savings: $${estimatedSavings.toFixed(2)}.`
  }

  if (finalClassification === "yellow") {
    return `We found a possible ${issue} issue on this bill. Please review before paying while we confirm next steps. Estimated savings: $${estimatedSavings.toFixed(2)}.`
  }
...
function derivePrimaryClassification(findings: ClassifiedFinding[], shadowState: boolean): FinalClassification {
...
function mapPaymentRecommendation(primary: FinalClassification): PaymentRecommendationFinal {
...
function buildOverallRationale(
  ...
  if (primary === "red") {
    return `We found ${redCount} confirmed billing issue${redCount === 1 ? "" : "s"} on this bill. Do not pay yet while XILO prepares corrective action.`
  }

  if (primary === "yellow") {
    return `We found ${yellowCount} bill issue${yellowCount === 1 ? "" : "s"} that need confirmation or review before payment.`
  }
...
export function classifyClaim(
  ...
  const redCount = classifiedFindings.filter(f => f.final_classification === "red").length
  const yellowCount = classifiedFindings.filter(f => f.final_classification === "yellow").length
  const greenCount = classifiedFindings.filter(f => f.final_classification === "green").length
  const shadowCount = shadowState
    ? input.findings.length || 1
    : classifiedFindings.filter(f => f.final_classification === "shadow").length

  const totalRedSavings = round2(
    classifiedFindings
      .filter(f => f.final_classification === "red" && f.dispute_status === "ready")
  ...
  const totalFinalSavings = round2(
    classifiedFindings
      .filter(f => f.final_classification === "red" || f.final_classification === "yellow")
      .reduce((sum, f) => sum + f.estimated_savings_final, 0)
  )

  const disputeReadyCount = classifiedFindings.filter(f => f.dispute_status === "ready").length
  const truthLayerRequiredCount = classifiedFindings.filter(
    f => f.truth_layer_status === "eligible" || f.truth_layer_status === "queued"
  ).length
  const manualReviewCount = classifiedFindings.filter(
    f => f.dispute_status === "manual_review_required" || f.dispute_status === "shadow"
  ).length

  const primaryClassification = derivePrimaryClassification(classifiedFindings, shadowState)
  const billHealthState = mapBillHealthState(primaryClassification)
  const paymentRecommendationFinal = mapPaymentRecommendation(primaryClassification)
  const employeeAppStatus = mapEmployeeAppStatus(primaryClassification)
  const finalBillHealthScore = computeBillHealthScore(classifiedFindings, shadowState)
  ...
  const output: ClassificationOutput = {
    ...
    routing_recommendations: {
      ...
      recommended_dispute_channel:
        disputeReadyCount > 0 ? "email" : "none",
    },

    error_handling: {
      shadow_state: shadowState,
      shadow_reason: shadowState
        ? input.inherited_shadow_reason ?? "Inherited shadow state from upstream verification."
        : null,
    },
  }

  // Minimal server-side validations aligned to schema guidance
  const totalCount =
  ...
  const expectedLength = shadowState ? Math.max(input.findings.length, 1) : output.classified_findings.length
  if (totalCount !== expectedLength) {
    throw new Error("Classification count mismatch.")
  }

  for (const finding of output.classified_findings) {
    if (finding.employee_refund_eligible && finding.participation_reward_eligible) {
      throw new Error("A finding cannot have both refund and reward eligibility.")
    }

    if (finding.requires_truth_layer_final && finding.dispute_status !== "pending_truth_layer") {
      throw new Error("Truth-layer-required finding must have dispute_status = 'pending_truth_layer'.")
    }
  }
